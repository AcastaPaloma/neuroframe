// Adapted from NeLy-EPFL/flygym (Apache-2.0), revision 38c8ec61034cd59bc5ba0de20688d4a3c0000d60.
...
const loadMujoco = async () => { if (!globalThis.neuroframeMujoco) throw new Error('MuJoCo runtime did not load. Reload the experiment.'); return globalThis.neuroframeMujoco; };
export { loadMujoco };
...
export async function writeModelToFS(mj, { xmlText, xmlName = 'model.xml', modelBaseUrl, onProgress }) {
  ...
  await Promise.all(meshFiles.map(async (f) => {
    const buf = new Uint8Array(await fetch(`${modelBaseUrl}/${f}`).then((r) => { if (!r.ok) throw new Error(`Missing body mesh: ${f}`); return r.arrayBuffer(); }));
